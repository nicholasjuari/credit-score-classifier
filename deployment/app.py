"""
app.py — Streamlit App versi Lokal
==================================
Tampilan identik dengan aws_deployment/app_aws.py, tetapi memuat model
dari folder lokal artifacts/ (via CreditScorePredictor), bukan dari S3.
...
from inference import CreditScorePredictor  # noqa: E402
...
try:
    ...
    st.error(f"Gagal load model dari folder artifacts/:\n\n{e}\n\n"
             f"Pastikan file best_model.pkl, label_encoder.pkl, dan "
             f"metadata.pkl ada di folder artifacts/. Jalankan training "
             f"dulu jika belum ada.")
    st.stop()


with st.sidebar:
    st.markdown("### 💳 Model Deployment")
    st.markdown("**Source**: `artifacts/` (local)")
    st.markdown("---")
    st.caption("Nicholas Juari · 2802413064")


st.markdown('<div class="main-header">💳 Credit Score Classifier</div>',
            unsafe_allow_html=True)
